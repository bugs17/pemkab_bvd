...
import { formatISOToDateTimeLocal } from "@/app/lib/formatedIsoToLocal";
import { editKegiatan } from "@/app/actions/edit-kegiatan";

const FormEditKegiatan = ({ judulProps, deskripsiProps, id, mulaiProps, selesaiProps }) => {
  ...
  useEffect(() => {
    setJudul(judulProps);
    setDeskripsi(deskripsiProps)
    setMulai(formatISOToDateTimeLocal(mulaiProps))
    setSelesai(formatISOToDateTimeLocal(selesaiProps))
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await editKegiatan(judul, deskripsi, mulai, selesai, id);

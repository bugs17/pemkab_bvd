...
const ButtonChangePassword = ({userId, username}) => {
    ...
    const handleSubmit = async () => {
        ...
        const res = await changeUserPasswordClerk(userId, newPassword)
        if (res){
            alert(`Password untuk user ${username} telah berhasil diubah 👍🏻`)
            document.getElementById(`modal_changepass_user${userId}`).close()
            setNewPassword("")
            setIsSubmit(false)
            return
        }
  ...
  return (
    <>
        ...
        <dialog id={`modal_changepass_user${userId}`} className="modal">

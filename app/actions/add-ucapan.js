"use server";

import { writeFile } from "fs/promises";
import { prisma } from "../lib/db";
import path from "path"
import { revalidatePath } from "next/cache";

export const addUcapan = async (file) => {
    if (!file) {
        return false
    }

    try {
        // Modifikasi nama file: hilangkan spasi dan tambahkan timestamp
        const timestamp = Date.now()
        const originalName = file.name.replace(/\s+/g, "") // Hapus semua spasi
        const extension = path.extname(originalName) // Ekstensi file (.jpg, .png, dll)
        const fileName = `${path.basename(originalName, extension)}-${timestamp}${extension}`
    
        // Tentukan lokasi penyimpanan file
        const filePath = path.join(process.cwd(), "/uploads/ucapan", fileName)
    
        // Simpan file ke server
        const namaFileDiDb = `${fileName}`
        await writeFile(filePath, Buffer.from(await file.arrayBuffer()))
    
        await prisma.ucapan.create({
            data:{
                urlFile:namaFileDiDb
            }
        })
        revalidatePath("/admin/setting-web/ucapan")
        revalidatePath("/")
        return true
    } catch (error) {
        console.log("Terjadi error saat add ucapan ke server", error.message)
        return false
    }
}